...
public class BulleBehavior : MonoBehaviour
{
    ...
    public float noiseFrequencyIncrement = 1f; // Fréquence des variations aléatoires lors de l'incrémentation
    public float noiseFrequencyDecrement = 0.5f; // Fréquence des variations aléatoires lors de la décrémentation
    ...
    void Update()
    {
        ...
        {
            isFleeing = true;
            ApplyRandomDirection(); // Envoie dans une direction aléatoire
        }
        ...
        if (isFleeing)
        {
            ...
            {
                MoveTowards(currentTarget, noiseFrequencyDecrement); // Continue de se déplacer vers la cible avec le bruit de décrémentation
            }
        }
        else
        {
            ApplyFloatingMotion(noiseFrequencyIncrement); // Mouvement flottant imprévisible avec bruit d'incrémentation
        }
    }
    ...
    // Calcul de l'offset pour le mouvement flottant imprévisible, en prenant en compte la fréquence du bruit
    ...
    // Applique une force dans une direction aléatoire
    private void ApplyRandomDirection()
    {
        ...
        // Applique une impulsion dans la direction aléatoire
